...
function createToken(user) {
  return jwt.sign(_.omit(user, 'CONTRASENIA'), config.secretKey, { expiresIn: 60*60*5 });
}
...
function getUserDB(username, done) {
  ...
  var str='SELECT * FROM usuarios WHERE user = "'+username+'"';
  console.log(str);
  try{
...
app.post('/user/login', function(req, res) {
  ...
  getUserDB(req.body.username, function(user){
    ...
    if (user.CONTRASENIA !== req.body.password) {
...
app.get('/user/verify', jwtCheck, function(req, res){
  var user = req.user;
        db.get().query('SELECT * FROM usuario WHERE user = ? LIMIT 1', [user.username], function(err, rows) {
            if(err){
